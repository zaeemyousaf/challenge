...
// working fine except deleteKey which is incomplete
// rest is ok
#include <iostream>
using namespace std;
...
private:
  ...
  void delete_nodes(TNode<k,v>* temp);
  void count(int& n, TNode<k,v>* temp);
public:
  BST();
...
int BST<k,v>::length() {
    ...
    if (this->root == NULL) {
      ...
      if (root->rightChild != NULL) {
	count(total, root->rightChild);
      }
      if (root->leftChild != NULL) {
	count(total, root->leftChild);
      }
    }
...
template <typename k, typename v>
void BST<k,v>::count(int& n, TNode<k,v>* temp) {
    n++;
    if (temp->rightChild != NULL) {
      count(n, temp->rightChild);
    }
    if (temp->leftChild != NULL) {
      count(n, temp->leftChild);
    }
  }

//--------------
template <typename k, typename v>
void BST<k,v>::deleteKey(k key){
...
template <typename k, typename v>
void BST<k,v>::delete_nodes(TNode<k,v>* temp) {
    if (temp->leftChild != NULL) {
      delete_nodes(temp->leftChild);
    }
    if (temp->rightChild != NULL) {
      delete_nodes(temp->rightChild);
    }
    TNode<k,v>* Del = temp;
    temp = NULL;
    delete Del;
